...
# Determine whether the framework is running in Lite mode.  When
# NR_LITE_MODE is set to 1 any heavy scans should be replaced with
# lightweight alternatives, thread counts reduced and wordlists
# truncated.  Many modules check this helper to adjust their
# behaviour.  It returns 0 (success) when lite mode is active and
# 1 (failure) otherwise so that it can be used in conditionals.
is_lite() {
    [[ "${NR_LITE_MODE:-0}" == "1" ]]
}

# Helper to select an appropriate wordlist.  Modules that support
# wordlists should call this instead of hard‑coding a path.  In
# normal mode the default provided by the module is returned.  In
# lite mode the function attempts to choose a smaller dictionary to
# minimise resource usage.  Callers should always check that the
# returned path exists before use.  Args: $1 = default wordlist path.
get_wordlist() {
    local default_list="$1"
    if is_lite; then
        # Look for a smaller common wordlist.  Prefer SecLists' xato-net
        # top 1000 list if present, otherwise fall back to the first 1000
        # lines of the default list written to a temporary file.
        local small_candidates=(
            "/usr/share/seclists/Passwords/xato-net/10-million-passwords-top-1000.txt"
            "/usr/share/wordlists/rockyou.txt"
        )
        for candidate in "${small_candidates[@]}"; do
            if [[ -f "$candidate" ]]; then
                # If the candidate is the default list pick the top 1000 entries
                if [[ "$candidate" == "$default_list" ]]; then
                    local tmp="${TMPDIR:-/tmp}/netreaper_lite_$(basename "$candidate").lst"
                    if [[ ! -f "$tmp" ]]; then
                        head -n 1000 "$candidate" > "$tmp" 2>/dev/null || true
                    fi
                    echo "$tmp"
                    return 0
                fi
