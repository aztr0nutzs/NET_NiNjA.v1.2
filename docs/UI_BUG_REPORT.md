...
## Verification
- Manual checks: Launch app, verify WebView loads `ninja_mobile_new.html` and stays open.
- Smoke tests: Not run (manual only).

## Update: Engine Offline On Android 14+
- User-visible symptom: Dashboard never loads and the local engine is unreachable.
- Root cause: Foreground engine service lacked a declared service type, which can prevent startForeground on Android 14 (targetSdk 34).
- Fix: Add `android:foregroundServiceType="dataSync"` and request `FOREGROUND_SERVICE_DATA_SYNC` permission.

## Update: Dashboard Wiring + Scan Inert
- User-visible symptom: App loads the wrong dashboard and SCAN actions do nothing.
- Root cause: WebView opened the `new_assets` dashboard while the intended UI lived in `ninja_mobile_new.html`; relative `/api/*` calls broke under `file://` on the bootstrap page.
- Fix: Switch entry points to `ninja_mobile_new.html` and prepend the API base for file-scheme loads.
